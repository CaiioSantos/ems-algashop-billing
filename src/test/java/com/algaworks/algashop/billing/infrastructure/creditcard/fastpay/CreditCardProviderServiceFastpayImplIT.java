...
@SpringBootTest
@ActiveProfiles("test")
@Import({FastpayCreditCardTokenizationAPIClientConfig.class, FastpayCreditCardAPIClientConfig.class})
class CreditCardProviderServiceFastpayImplIT {
    ...
    public void shouldDeleteRegisteredCreditCard() {
        ...
        // Adiciona pequeno delay para garantir que o cartão foi registrado no backend
        try {
            Thread.sleep(100);
        ...
        // Adiciona pequeno delay após deletar
        try {
            Thread.sleep(100);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
